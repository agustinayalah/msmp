...
main(int argc, char *argv[]) {
	...
	if (argc > 1)
	    printf("pi: %lf ss: %d  D: %lf H: %lf thetah: %lf segsub: %d \n", pi, segsites,
		   tajd(nsam, segsites, pi), h, th, nsegsub);
	else if (probflag == 1)
	    printf("pi:\t%lf\tss:\t%d\tD:\t%lf\tthetaH:\t%lf\tH:\t%lf\tprob:\t%g%s", pi, segsites,
		   tajd(nsam, segsites, pi), th, h, prob, slashline);
	else
	    printf("pi:\t%lf\tss:\t%d\tD:\t%lf\tthetaH:\t%lf\tH:\t%lf%s", pi, segsites,
		   tajd(nsam, segsites, pi), th, h, slashline);
    }
}
