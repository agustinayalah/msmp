...
*	Modifications to add // before each sample  25 Feb 2001
        ...
        !!! Fixed bug in -en option.  Earlier versions may have produced garbage when -en ... used.
9 Dec 2003 Fixed bug which resulted in incorrect results for the case where rho = 0.0 and gene
conversion rate > 0.0. This case was not handled correctly in early versions of the program. 5 Apr
2004.  (Thanks to Vincent Plagnol for pointing out this problem.) Fixed bug in prtree().  Earlier
versions may have produced garbage when the -T option was used. 1 Jul 2004. Fixed bug in -e. options
that caused problems with -f option  13 Aug 2004. Fixed bug in -es option, which was a problem when
used with -eG. (Thanks again to V. Plagnol.) 6 Nov. 2004 Added -F option:  -F minfreq  produces
output with sites with minor allele freq < minfreq filtered out.  11 Nov. 2004. Fixed bug in
streec.c (isseg() ).  Bug caused segmentation fault, crash on some machines. (Thanks to Melissa Jane
Todd-Hubisz for finding and reporting this bug.) Added -seeds option 4 Nov 2006 Added "tbs"
arguments feature 4 Nov 2006 Added -L option.  10 May 2007 Changed -ej option to set Mki = 0
pastward of the ej event.  See msdoc.pdf.  May 19 2007. fixed bug with memory allocation when using
-I option. This caused problems expecially on Windows machines.  Thanks to several people, including
Vitor Sousa and Stephane De Mita for help on this one. Oct. 17, 2007. Modified pickb() and pickbmf()
to eliminate rare occurrence of fixed mutations Thanks to J. Liechty and K. Thornton. 4 Feb 2010.
...
this suggestion.) Changed function definitions, and misc other things to comply with c99
...
#pragma omp threadprivate(maxsites, posit, agevec, alleleage)
...
/*
*	@brief Aloca los buffers de una replica (ms_list/posit/agevec) para el hilo que la está llamando. 
*   Debe invocarse una vez por hilo, antes de simular cualquier réplica en ese hilo.
*/
...
/*
*	@brief Simula una réplica construyendo el arbol con gensam y aplicando mutaciónes e imprime su resultado en pf.
*/
void run_one_replicate(FILE *pf)
{
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if (ntbs > 0){
        ...
        }   // Termina la región paralela, join de hilos
        for (i=0 ; i < howmany ; i++){
            fputs(out_bufs[i], pf) ;
			free(out_bufs[i]) ;
        }
    ...
    while (arg < argc) {
        ...
        case 'I':
            ...
            if (count == 0) {
                ...
                pars.cp.config =
                    (int *)realloc(pars.cp.config, (unsigned)(pars.cp.npop * sizeof(int)));
                npop = pars.cp.npop;
            }
            arg++;
            ...
            if (count == 0) {
                pars.cp.mig_mat = (double **)realloc(pars.cp.mig_mat,
                                                     (unsigned)(pars.cp.npop * sizeof(double *)));

                pars.cp.mig_mat[0] = (double *)realloc(pars.cp.mig_mat[0],
                                                       (unsigned)(pars.cp.npop * sizeof(double)));
                ...
                pars.cp.size =
                    (double *)realloc(pars.cp.size, (unsigned)(pars.cp.npop * sizeof(double)));
...
void usage()
{
    ...
    fprintf(stderr, "\t\t -c f track_len   (f = ratio of conversion rate to rec rate. tracklen is "
                    "mean length.) \n");
    ...
    fprintf(stderr, "\t\t -ma m_11 m_12 m_13 m_21 m_22 m_23 ...(Assign values to elements of "
                    "migration matrix.)\n");
    fprintf(stderr, "\t\t -n i size_i   (popi has size set to size_i*N0 \n");
    fprintf(stderr, "\t\t -g i alpha_i  (If used must appear after -M option.)\n");
    fprintf(stderr, "\t   The following options modify parameters at the time 't' specified as the "
                    "first argument:\n");
    ...
    fprintf(stderr, "\t -ema t npop  m_11 m_12 m_13 m_21 m_22 m_23 ...(Assign values to elements "
                    "of migration matrix.)\n");
    ...
    fprintf(stderr, "\t\t proportion is probability that each lineage stays in pop-i. (p, 1-p are "
                    "admixt. proport.\n");
    fprintf(stderr, "\t\t Size of pop npop is set to N0 and alpha = 0.0 , size and alpha of pop i "
                    "are unchanged.\n");
    ...
    fprintf(stderr, "\t  -p n ( Specifies the precision of the position output.  n is the number "
...
int pickb(int nsam, struct node *ptree, double tt)
{
    ...
    for (i = 0, y = 0; i < 2 * nsam - 2; i++) {
        ...
        if (y >= x) {
            alleleage =
                (ptree + i)->time + ran1() * ((ptree + (ptree + i)->abv)->time - (ptree + i)->time);
            return (i);
        }
    }
