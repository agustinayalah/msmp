...
int main(int argc, char *argv[]){
    ...
    if (ntbs > 0)
	for (k = 0; k < ntbs; k++) scanf(" %s", tbsparamstrs[k]);
	...
	posit = (double *)malloc(
	    (unsigned)(maxsites *
		       sizeof(
			   double)));  // posit y agevec son los metadatos de los segmentos como la
...
void getpars(int argc, char *argv[], int *phowmany){
	...
	*phowmany = atoi(argv[2]);
	...
	switch (argv[arg][1]) {
		...
		if (count == 0) {
		    pars.cp.mig_mat = (double **)realloc(
			pars.cp.mig_mat, (unsigned)(pars.cp.npop * sizeof(double *)));

		    pars.cp.mig_mat[0] = (double *)realloc(
			pars.cp.mig_mat[0], (unsigned)(pars.cp.npop * sizeof(double)));
		    ...
		    pars.cp.alphag = (double *)realloc(pars.cp.alphag,
		...
		palpha = atof(argv[arg++]);
		for (i = 0; i < pars.cp.npop; i++) pars.cp.alphag[i] = palpha;
		break;
	    case 'e':
...
void usage(){
    ...
    fprintf(stderr,
	    "\t\t -c f track_len   (f = ratio of conversion rate to rec rate. tracklen is "
	    "mean length.) \n");
    ...
    fprintf(stderr,
	    "\t\t -ma m_11 m_12 m_13 m_21 m_22 m_23 ...(Assign values to elements of "
	    "migration matrix.)\n");
    fprintf(stderr, "\t\t -n i size_i   (popi has size set to size_i*N0 \n");
    fprintf(stderr, "\t\t -g i alpha_i  (If used must appear after -M option.)\n");
    fprintf(stderr,
	    "\t   The following options modify parameters at the time 't' specified as the "
	    "first argument:\n");
    ...
    fprintf(stderr,
	    "\t -ema t npop  m_11 m_12 m_13 m_21 m_22 m_23 ...(Assign values to elements "
	    "of migration matrix.)\n");
    ...
    fprintf(stderr,
	    "\t\t proportion is probability that each lineage stays in pop-i. (p, 1-p are "
	    "admixt. proport.\n");
    fprintf(stderr,
	    "\t\t Size of pop npop is set to N0 and alpha = 0.0 , size and alpha of pop i "
	    "are unchanged.\n");
    ...
    fprintf(stderr,
	    "\t  -p n ( Specifies the precision of the position output.  n is the number "
...
double ttime(struct node *ptree, int nsam){
    ...
    /* adna */
    for (i = 0; i < nsam; i++) t -= (ptree + i)->time;
